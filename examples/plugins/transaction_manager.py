...
class TransactionManager:
    ...
    def __init__(self):
        ...
        self.transactions_filename = (
            "transactions_" + datetime.fromtimestamp(time()).strftime("%Y_%m_%d_%H_%M_%S") + ".csv"
        )

        # fields set by default in jmeter
        self.csv_headers = ["start_time","duration","transaction_name","user_count","success","failure_message","transaction_id"]
    ...
    def end_transaction(self, transaction_id, success=True, failure_message=""):
        ...
        csv_row = f'{datetime.fromtimestamp(t["start_time"]).strftime(self.timestamp_format)}{self.field_delimiter}{round(t["duration"],2)}{self.field_delimiter}{t["transaction_name"]}{self.field_delimiter}{t["user_count"]}{self.field_delimiter}{t["success"]}{self.field_delimiter}{t["failure_message"]}{self.field_delimiter}{t["transaction_id"]}'
    ...
    def _on_locust_init(self, environment, runner, **kwargs):
        ...
        if self.env.web_ui:
            ...
            # "Average Content Size","Requests/s","Failures/s","50%","66%","75%","80%","90%","95%","98%","99%","99.9%","99.99%","99.999%","100%"
            @self.env.web_ui.app.route("/stats/transactions/csv")
            def transactions_summary_page():
                #make some summaries from the transactions
                resp = [] 
                resp.append('"Name","Request Count","Median Response Time","Average Response Time","Min Response Time","Max Response Time"')
                for tname in self.completed_transactions:
                    ...
                    resp.append(f'{tname}{self.field_delimiter}{len(durations)}{self.field_delimiter}{avg}{self.field_delimiter}{round(sorted_durations[0],2)}{self.field_delimiter}{round(sorted_durations[-1],2)}')
