...
class TransactionManager:
    ...
    def __init__(self):
        ...
        self.test_timestamp = datetime.fromtimestamp(time()).strftime(
            "%Y_%m_%d_%H_%M_%S"
        )
        self.transactions_filename = f"transactions_{self.test_timestamp}.csv"
        self.transactions_summary_filename = (
            f"transactions_summary_{self.test_timestamp}.csv"
        )
    ...
    def _write_final_log(self, **kwargs):
        if not self.is_not_worker:  # fewer nots overall this way round...
            print("going to make final report to master")
            self._report_to_master()
        else:
            if self.log_transactions_in_file:
                ...
                summary_file.write(
                    self.row_delimiter.join(self.get_transactions_summary())
                )
                summary_file.close()
    ...
    def _on_locust_init(self, environment, runner, **kwargs):
        ...
        if self.env.web_ui:
            ...
            def transactions_results_page():
                ...
                headers[
                    "Content-disposition"
                ] = f"attachment;filename={self.transactions_filename}"
            ...
            def transactions_summary_page():
                ...
                headers[
                    "Content-disposition"
                ] = f"attachment;filename={self.transactions_summary_filename}"
                response = self.env.web_ui.app.response_class(
                    response=self.row_delimiter.join(response_body),
                    headers=headers,
                    status=200,
                    mimetype="text/csv",
                )
